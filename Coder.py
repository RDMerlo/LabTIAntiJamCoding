...
class cWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.tableWidget4.setRowCount(1)
    ...
    def btn1Clicked(self):
        ...
        self.ui.tableWidget1.setColumnCount(self.cCode.m)
        ...
        self.ui.tableWidget3.setColumnCount(self.cCode.n)
        for k in range(self.cCode.n):
            cellInfo = QtWidgets.QTableWidgetItem('')
            cellInfo.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            self.ui.tableWidget3.setItem(0, k, cellInfo)
        # Очистим строкy b + e
        self.ui.tableWidget4.clearContents()
        self.ui.tableWidget4.setColumnCount(self.cCode.n)
        for k in range(self.cCode.n):
            cellInfo = QtWidgets.QTableWidgetItem('')
            cellInfo.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            self.ui.tableWidget4.setItem(0, k, cellInfo)
        pass
    ...
    def btn3Clicked(self):
        ...
        for k in range(self.cCode.n):
            try:
                i = int(self.ui.tableWidget2.item(0, k).text().strip())
                if not (i in [0,1]):
                    attention2('b')
                    i = 0 # Значение по умолчанию
            except ValueError:
        ...
        for k in range(self.cCode.n):
            try:
                i = int(self.ui.tableWidget3.item(0, k).text().strip())
                if not (i in [0,1]):
                    attention2('e')
                    i = 0 # Значение по умолчанию
            except ValueError:
        ...
        self.ui.tableWidget3.setColumnCount(self.cCode.n)
        for k in range(self.cCode.n):
            cellInfo = QtWidgets.QTableWidgetItem(str(e[k]))
            cellInfo.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            self.ui.tableWidget3.setItem(0, k, cellInfo)
        # Найдем строку c = b + e
        c = self.cCode.makeError(b,e)
        # Запишем строкy c = b + e
        self.ui.tableWidget4.clearContents()
        self.ui.tableWidget4.setColumnCount(self.cCode.n)
        for k in range(self.cCode.n):
